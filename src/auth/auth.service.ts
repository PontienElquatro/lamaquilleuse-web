...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    const user = await this.prisma.user.create({
      data: {
        ...
        role: dto.role as any, provider: 'LOCAL' as any,
        emailVerifyToken: verifyToken, isEmailVerified: false,
      },
    });
  ...
  async login(dto: LoginDto) {
    ...
    if (!user.password) throw new UnauthorizedException(`Ce compte utilise la connexion via ${user.provider}`);
  ...
  async forgotPassword(email: string) {
    ...
    await this.prisma.user.update({ where: { id: user.id }, data: { resetPasswordToken: resetToken, resetPasswordExpiry: tokenExpiry } });
    this.mail.sendResetPasswordEmail(user.email, user.firstName, resetToken).catch(() => {});
    return { message: 'Si cet email existe, un lien a été envoyé' };
  }

  async resetPassword(token: string, newPassword: string) {
    const user = await this.prisma.user.findFirst({ where: { resetPasswordToken: token, resetPasswordExpiry: { gt: new Date() }, deletedAt: null } });
    if (!user) throw new BadRequestException('Token invalide ou expiré');
    const hashed = await bcrypt.hash(newPassword, 12);
    await this.prisma.user.update({ where: { id: user.id }, data: { password: hashed, resetPasswordToken: null, resetPasswordExpiry: null, refreshToken: null } });
  ...
  async handleGoogleLogin(profile: GoogleProfile, defaultRole: Role = Role.CLIENT) {
    ...
    if (!user) {
      user = await this.prisma.user.create({
        data: { email: profile.email, firstName: profile.firstName, lastName: profile.lastName, avatar: profile.avatar, provider: 'GOOGLE' as any, providerId: profile.providerId, role: defaultRole as any, isEmailVerified: true },
      });
      this.mail.sendWelcomeEmail(user.email, user.firstName).catch(() => {});
    }
    const tokens = await this.generateTokens(user.id, user.email, user.role);
  ...
  async deleteAccount(userId: string) {
    await this.prisma.user.update({ where: { id: userId }, data: { deletedAt: new Date(), refreshToken: null, email: `deleted_${userId}@lamaquilleuse.fr` } });
